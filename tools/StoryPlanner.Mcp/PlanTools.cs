...
public sealed class PlanTools(StoryPlanSources sources)
{
    ...
    [Description("Group and count WORKING plan (v2) notes by up to 3 dimensions: state, track, trackType, ownerType, subject, subjectType, chapter, story, theme, hasWorldDate, theater, dateShape. \"story\" resolves Chapter/PlotPoint/Link-owned notes through to their story (\"(Unassigned)\" if the chapter has none); Subject notes report \"(no story)\". \"theater\" is the timeline column a note renders in, via its owning subject's authorial placement (\"(Unplaced)\" = not placed, a legal state). \"dateShape\" is how the note draws on the timeline — event at year/month/day precision vs condition by span length vs open-ended vs undated. Counts include flagged notes (counts are numbers, not content). Compose absence/intersection questions from counts — e.g. groupBy [\"subject\",\"trackType\"] then look for zeros.")]
