...
/// kind label is the page's; the text never repeats it.
/// </summary>
public static class StreamEvents
{
    ...
    /// steps collapsed to one line first; empty when the file does not exist yet.
    /// </summary>
    ...
    /// running total, with the step count appended. Every other event passes through.
    /// </summary>
    public static IReadOnlyList<StreamEvent> Collapse(IEnumerable<StreamEvent> events)
    {
        ...
        {
            if (e.Kind == "thinking") { pendingThinking = e; steps++; continue; }
            Flush();
            result.Add(e);
        }
        Flush();
