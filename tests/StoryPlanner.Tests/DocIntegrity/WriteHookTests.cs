...
public class WriteHookTests
{
    ...
    public void A_file_inside_a_skill_folder_with_an_artifacts_table_is_governed()
    {
        ...
        var located = GovernedSkill.Locate(Path.Combine(f.SkillFolder, MapFixture.ArtifactsFile));
    ...
    [Fact]
    public void A_skill_folder_without_an_artifacts_table_is_not_governed()
    {
        using var f = MapFixture.Without(MapFixture.ArtifactsFile);
        Assert.Null(GovernedSkill.Locate(Path.Combine(f.SkillFolder, MapFixture.SkillFile)));
    }
