# CLAUDE.md as it stood on 2026-09-12, before the split into skills

> **HISTORICAL REFERENCE — do not update this file.** This is a verbatim copy of `CLAUDE.md`
> at 6,727 words, taken immediately before its content was distributed into skills. It had
> grown 5.2× in six weeks across 25 commits, none of which ever reduced it, and two sections —
> *Data semantics* and *Build & run* — were 68% of it.
>
> Where it went: the data-semantics traps to `storyplan-data`; the sidecar corpora to `corpora`;
> build, publish and run to `build-and-run`; the architecture and namespace rules to
> `code-conventions`; the WPF verification handoff was already in `testing`. `CLAUDE.md` kept
> the rules that every session needs before it reads anything else, plus routing.
>
> **Where this file and the live text disagree, the live text is authoritative.** This is
> kept because the moved paragraphs carry dated decisions and their reasoning, and a paragraph
> that lost a clause in the move is recoverable here.
