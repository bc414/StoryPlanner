...
| **Codebooks as instruments** (first placed in `docs/v3-framework/codebooks/`; that folder was dissolved the same evening — a codebook now lives with the work that calibrates it under `fanout/`, see the addendum below) — HITL-authored, calibrated on Brian's verdicts, hash-versioned, re-run on revision | Classification embedded in per-story analyses under a brief | The 046 DT-class classification in WU1.9's spec is the template. |
| **The referee has a codebook** (now `fanout/referee/codebook.md`, with its calibration record as `calibration-<date>.md` beside it: three inputs, three classes, decision rules R1–R9, worked examples, calibration requirement) and the skill only states what the codebook must implement | The referee's procedure as prose in the skill — a document the explicit-context rule says the referee never sees | Drafted 2026-09-03 at Brian's prompt ("don't we need a codebook for this procedure instead of just text in a skill?"); **uncalibrated** until a calibration record exists, which now precedes the retroactive pass. |
...
**Rulings closing the "Owed" list, 2026-09-03 (night).** The adversarial comparison ran and
the name settled (above). The remaining rulings, all Brian's, are recorded in
`forward-plan-2-handoff.md` § Rulings made 2026-09-03: the leftover `/analyze-story`
transcripts stay where they are (insignificant); the rule-matching sessions in
codesessions.db are purged by a one-time script (`tools/StoryPlanner.CodeSessions/scripts/
purge-excluded-2026-09-03.sql`, selecting by the ingest's own predicate); hypothesis 013's
refinement stays an iteration. Two inconsistencies this note carried were corrected at the
same time: the codebook's calibration file is `calibration-<date>.md` (the referee codebook
had said `referee.calibration-<date>.md`, a name the runner's stage detector would never have
matched), and the `codebooks/` folder references in the table above now point at where the
files went.
