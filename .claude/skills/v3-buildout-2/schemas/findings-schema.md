...
|---|---|---|---|
| `question` | optional | token of question-list | the frozen question this finding answers, one at most, `questions/<slug>`, a token in the directions version's frontmatter; absent for a finding the data raised; what the tool derives a question's answered state from |
...
per line; none is a question until Brian writes it into the question list, and none is a
finding.
...
- question: questions/scene-notes-carry-designed-stasis
...
| every finding that answers one question | `grep -l '^- question: questions/<slug>' studies/*/findings.md` |
...
| `findings.question` | a question token is not `questions/<slug>` naming an entry in the question list, or is not in the frontmatter of the directions version the study's batches name |
