...
index is written by hand, and nothing in it is derived from a result. The head names no corpus:
an itemizer reads corpora and nothing else, one or several, and which ones is in its code at the
version its head line names, the `locator notation` line saying in words what a locator
addresses; a collator reads the method's own files its activity names. No state of an input is
recorded.
...
| which tool wrote a batch | the head's first line: `itemizer` or `collator` |
| which corpora a batch's items were cut from | the `itemizer` line names the tool and its version, whose code at that version is what it read; the head's `locator notation` says in words what a locator addresses |
...
| `index.head` | a head key is missing, unknown or out of order; both or neither of `itemizer` and `collator` are present; `source hash` is present and not a SHA-256 |
