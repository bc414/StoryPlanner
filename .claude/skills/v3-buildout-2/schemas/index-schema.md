...
| `utilizes corpora` | optional | list of enum: the ids in CORPORA.md | the other corpora the itemizer read to cut, label or fill the items; never the `corpus` value |
| `utilizes outputs` | optional | list of line | the deterministic outputs over corpus data the itemizer read, one repo-relative path per item |
| `narrowing` | optional: when the itemizer does not cut every item of its corpus | line | in words, the condition in the itemizer's code that chooses which of the corpus's items the batch holds |
...
in the index is written by hand, and nothing in it is derived from a result. An itemizer
whose `corpus` is in CORPORA.md reads corpus data only, other corpora and deterministic
outputs over corpus data among it, and the two `utilizes` keys name what it read besides
the corpus it cut; no state of an input is recorded.
...
| every batch whose itemizer utilized an output | `grep -rln '^  - <path>$' studies/*/batches/*/index.md` |
| what a batch holds of its corpus | the head's `narrowing` line; absent, every item |
...
| `index.head` | a head key is missing, unknown or out of order; `corpus` is not an id in CORPORA.md, `candidates` or `skill`; a `utilizes corpora` value is not an id in CORPORA.md or repeats `corpus`; `utilizes outputs` names no output, or a path that does not exist under the repository root; `source hash` is present and not a SHA-256 |
