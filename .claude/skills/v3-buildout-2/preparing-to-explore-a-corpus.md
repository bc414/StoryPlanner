...
of verifications already promoted. If its question list is empty, Brian's opening question is
...
the session appends the study to `studies.md` with the date as his go. With arms it
writes `arm-key.md` and does not open it again.

## slice

A tool cuts the corpus into slices, one file each, with a manifest, under
`fanout/<study>/<run>/items/`; the runner's split verb where the corpus is markdown
units, otherwise a script with tests. The cut is mechanical and recorded in the manifest.

## author-protocol

Written against the real slices, with the questions in view verbatim and the lead-set
form the joiner expects. A new numbered
file.

## pilot-run

One job, one slice, under the protocol's hash, through the runner per the `agent-runner`
skill. Brian reads its lead set and says whether the protocol produces leads of the
...
Instructed by the reading protocol and nothing else; one slice in, one lead set out.
