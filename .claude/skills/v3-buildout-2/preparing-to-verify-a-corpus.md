...
`fanout/<study>/<run>/items/` and the manifest. The item definition it implements is
...
Runs a batch under a version with no calibration; shows Brian the agent's scores
