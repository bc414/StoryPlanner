...
The study is in the registry. The directions version the batch will name has an accepting
calibration at its body hash, and the itemizer that cut the calibration sample exists as a
tool. The study's question is named in that version's frontmatter.
...
result through the tally and the index, with the question the version's frontmatter names
in view, using `tally-batch --group-by` for any cross-tab it needs, and writes what the
data shows: one finding per thing shown, a count over the items, a pattern across items, a
contrast between classes, an answer that is null stated as plainly as one that is not. A
finding that answers the question names it; one the data raised names none. Each cites
what it rests on, the tally section as
...
about the study and never about the corpora; the question it raises is Brian's, in
