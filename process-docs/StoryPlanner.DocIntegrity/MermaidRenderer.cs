...
{
    const string Stamp =
        "> **UNVALIDATED** — rendered with `--force` while `check` still fails. Not authoritative.\n\n";
    ...
    public static string Map(SkillDocument doc, ValidationReport report, bool forced)
    {
        ...
        sb.Append("Never hand-edited: `render` rewrites this file whole from the tables in SKILL.md and the " +
                  "activity files, and the write hook rewrites it after every passing check. " +
