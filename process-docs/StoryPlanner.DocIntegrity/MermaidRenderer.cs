...
/// <summary>
/// Generates the marked sections: <c>level-1</c> in SKILL.md (the activities and their
/// <c>enables</c> edges), <c>activity</c> in each activity file (its processes, the artifacts
/// they touch, and what the tables derive for the activity), and the whole of <c>map.md</c>.
/// Never hand-edited, and deterministic: the same rows produce byte-identical text, so a diff
/// is a row change and never a re-render.
///
...
{
    public const string Level1Section = "level-1";
    ...
    public static string Level1(SkillDocument doc, bool forced)
    {
    ...
    public static string Activity(SkillDocument doc, string activityId, bool forced)
    {
    ...
    public static string Map(SkillDocument doc, ValidationReport report, bool forced)
    {
        ...
        sb.Append("```\n").Append(MapTables.GeneratedClose).Append("\n\n");

        sb.Append("<!-- generated:consumers -->\n## Consumers\n\n");
        ...
        foreach (var t in GraphRules.Traffic(doc))
            sb.Append("| ").Append(t.ArtifactId)
              ...
              .Append(" |\n");
        sb.Append(MapTables.GeneratedClose).Append("\n\n");
