...
/// <summary>
/// Writes <c>state.md</c>: what is here, read from the instance registry and the files on
/// disk, never what to do next. Three sections, ruled 2026-09-05:
///
/// - per instance, the instance-scoped artifacts present (paths from the Artifacts table,
...
{
    /// <summary>
    /// Chain membership by instance type, from artifacts.md § Instance registry. A closed set
    ...
    static readonly Regex RefereeInstance = new("^referee-[0-9]+$", RegexOptions.Compiled);
    ...
    public sealed record Instance(string Id, string Type, string Corpus, string Go);
    ...
    {
        /// <summary>Folder and version without the extension, at the short hash: the form the records cite.</summary>
    ...
    public static string Build(string repoRoot, SkillDocument doc, bool forced = false)
    {
        ...
        sb.Append("Never hand-edited: `state` rewrites this file whole from the instance registry, the question " +
                  "lists, the hypothesis files, the instance folders and the codebooks on disk. What is here, " +
                  "never what to do next; the pick is Brian's.\n\n");
        if (forced)
            sb.Append("> **UNVALIDATED** — written with `--force` while `validate` still fails. Not the record.\n\n");

        var questions = ReadQuestions(repoRoot, doc, out var questionsNote);
        Instances(sb, repoRoot, doc);
    ...
    static void Instances(StringBuilder sb, string repoRoot, SkillDocument doc)
    {
        sb.Append("## Instances\n\n");
        var row = doc.Artifact(WellKnown.Instances);
        if (row is null || !ArtifactPath.TryParse(row.Path, out var registryPath, out _))
        {
            sb.Append($"No `{WellKnown.Instances}` artifact row with a parseable path; the registry cannot be located.\n\n");
            return;
        }
        ...
        var instances = ReadRegistry(registry);
        ...
        {
            sb.Append($"### {inst.Id}\n\n");
            sb.Append($"- type: {inst.Type} · corpus: {inst.Corpus} · go: {inst.Go}\n");

            var chain = ChainFor(inst);
            if (chain is null)
            {
                sb.Append($"- chain: unknown type '{inst.Type}'; nothing derived\n\n");
                continue;
            }
            var missing = chain.Where(a => !doc.Activities.Any(x => x.Id == a)).ToList();
            var processes = chain.SelectMany(doc.ProcessesOf).ToList();
            var folder = InstanceFolder(inst.Id);
            ...
            var scoped = processes.SelectMany(p => p.Writes).Distinct(StringComparer.Ordinal)
                ...
                .Where(a => ArtifactPath.TryParse(a.Path, out var ap, out _) && ap!.IsInstanceScoped)
                .ToList();
            ...
            foreach (var a in scoped)
            {
                ...
                var count = CountMatches(repoRoot, ap!, folder, inst.Corpus);
                present[a.Id] = count;
                if (count == 0) continue;
                any = true;
                sb.Append($" {a.Id}");
                ...
                if (a.Id == WellKnown.Candidates)
                    foreach (var file in Matches(repoRoot, ap!, folder, inst.Corpus))
            ...
            sb.Append($"- furthest process whose instance-scoped writes all exist: {furthest ?? "none"}\n");
            if (missing.Count > 0)
    ...
    static IReadOnlyList<Instance> ReadRegistry(string path)
    {
        ...
        return table.Rows.Select(r => new Instance(r.Cells[0], r.Cells[1], r.Cells[2], r.Cells[3])).ToList();
    }

    static string[]? ChainFor(Instance inst)
    {
        if (RefereeInstance.IsMatch(inst.Id)) return RefereeChain;
    ...
    /// <summary>The referee's folder is <c>fanout/referee/</c> for every <c>referee-&lt;n&gt;</c> (artifacts.md § Instance registry).</summary>
    public static string InstanceFolder(string instanceId)
        => RefereeInstance.IsMatch(instanceId) ? "referee" : instanceId;
    ...
    public static IReadOnlyList<string> Matches(string repoRoot, ArtifactPath path, string? instanceFolder, string? corpus)
    {
        if (path.OutsideRepo) return [];
        var prefix = path.FixedPrefix(instanceFolder, corpus);
        var dir = prefix.Length == 0 ? repoRoot : Path.Combine(repoRoot, prefix.Replace('/', Path.DirectorySeparatorChar));
        if (!Directory.Exists(dir)) return [];

        var regex = path.ToRegex(instanceFolder, corpus);
    ...
    static int CountMatches(string repoRoot, ArtifactPath path, string? instanceFolder, string? corpus)
        => Matches(repoRoot, path, instanceFolder, corpus).Count;
    ...
    static void Corpora(StringBuilder sb, string repoRoot, SkillDocument doc, IReadOnlyList<Question> questions, string? note)
    {
        ...
        foreach (var corpus in questions.Select(q => q.Corpus).Distinct(StringComparer.Ordinal).OrderBy(c => c, StringComparer.Ordinal))
        {
            ...
            foreach (var q in open)
            {
                ...
                var answering = rounds.Where(r => r.Questions.Contains(q.Title, StringComparer.Ordinal)).Select(r => r.Instance).ToList();
    ...
    static IReadOnlyList<CodebookVersion> ReadCodebooks(string repoRoot, SkillDocument doc)
    {
        ...
        var calRow = doc.Artifact(WellKnown.CalibrationRecord);
        if (row is null || !ArtifactPath.TryParse(row.Path, out var path, out _)) return [];

        var accepted = new HashSet<string>(StringComparer.Ordinal); // hash prefixes with an accepting record
    ...
    sealed record Round(string Instance, IReadOnlyList<string> Questions);
    ...
    static IReadOnlyList<Round> ReadRounds(string repoRoot, SkillDocument doc)
    {
        ...
        foreach (var file in Matches(repoRoot, path!, null, null))
        {
            var questions = Section(File.ReadAllText(file), "Questions answered")
                ...
                .Where(l => l.Length > 0)
                .ToList();
            var instance = Path.GetFileName(Path.GetDirectoryName(file)!)!;
            result.Add(new Round(instance, questions));
        }
        return result;
    }
